...
class SmartDetection:
    def __init__(self):
        ...
        self.video_link = "http://{}:8000/video_feed".format(self.ipadd)
        self.sms_alert = "Alert: A person entered the room at {0}"
        self.sms_body = "Alert: A person entered the room at {0} and left the room at {1},click here for live video {2}"
        ...
        if self.confident >= 0.7:
            while True:
                ...
                if sum(self.de) == self.detection_thresh and not self.status:
                    ...
                    sms_alert_message = self.client.messages.create(
                        from_ = self.trial_Phone_num,
                        body = self.sms_alert.format(entry_time),
                        to = self.client_Phone_num,
                    )
                ...
                if self.status and not detected:
                    if sum(self.de) > (self.detection_thresh / 2):
                        ...
                        if time.time() - self.initial_time >= self.patience:
                            ...
                            self.send_message(self.sms_body.format(entry_time,exit_time,self.video_link),self.video_flag,attachment)
                            print()
                            print("***********************************************************")
                            print()
